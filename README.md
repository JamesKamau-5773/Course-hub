...
3. **Frontend Setup (Client)**:
   ...
   cd ..
   ```
   - Note: package.json includes a proxy to http://localhost:5000 for backend API calls
...
5. **Run the Application**:
   ...
   - Frontend: `cd client && npm start` (runs on http://localhost:3000, proxies API requests to backend)
...
```bash
# Reset database
rm coursehub.db  # or dogs.db if named differently
